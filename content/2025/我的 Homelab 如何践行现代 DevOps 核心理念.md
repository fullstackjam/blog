...
date = 2025-09-25
description = "三年 Homelab 实践总结：从雪花服务器到 GitOps，从手动部署到全自动化可观测性的完整演进。附架构图和踩坑记录。"
...
answer = "不需要很贵的设备。几台旧笔记本、迷你主机（如 Intel NUC）或者二手服务器都可以。我的集群从几台破服务器开始，关键是先跑起来，硬件可以慢慢升级。"

[[extra.faq]]
question = "Homelab 用 Kubernetes 会不会太重了？"
answer = "单纯跑几个容器确实不需要 K8s，Docker Compose 就够了。但如果你想系统地学习云原生技术栈，K8s 是绕不开的。而且一旦服务多了，K8s 的声明式管理和自动调度反而比手动管理省心。"

[[extra.faq]]
question = "GitOps 和传统 CI/CD 有什么区别？"
answer = "传统 CI/CD 是 push 模式：CI 构建完直接推到服务器。GitOps 是 pull 模式：集群里的 ArgoCD 持续监听 Git 仓库，发现变化就自动同步。好处是 Git 成为唯一的真相来源，所有变更都有记录，回滚就是 git revert。"

[[extra.faq]]
question = "Homelab 对职业发展有帮助吗？"
answer = "帮助很大。面试的时候能聊真实踩过的坑，而不是背概念。工作中遇到网络策略、存储卷、资源限制这些问题，因为在 Homelab 里都见过，定位速度快很多。最重要的是，你会形成一套 DevOps 思维方式，这比会用某个工具值钱得多。"

[[extra.faq]]
question = "从零开始搭 Homelab，推荐什么技术栈？"
answer = "建议分阶段：第一步用 Ansible 管理机器配置，实现 Infrastructure as Code；第二步上 K8s + ArgoCD，实现 GitOps 自动部署；第三步加 Prometheus + Grafana，实现可观测性。每一步都先跑通再进下一步，别一口气全上。"
+++
...
**最大的收获**：当基础设施变成代码以后，你就不再怕"搞坏"环境了。因为随时可以重来。代码在 Git 里，谁改了什么一目了然。
...
**最深的体会**：当部署变成 Git 操作时，运维工作的压力瞬间减少了一大半。不用记命令，不用怕手抖，出了问题 `git log` 就能定位。
...
- **能自动化的绝不手工操作** —— Ansible 管配置，ArgoCD 管部署，Renovate Bot 管依赖更新
- **能监控的绝不盲目运行** —— 没有指标的服务就是在裸奔
- **能版本化的绝不口头传递** —— 所有配置都在 Git 里，改了什么、为什么改、谁改的，一清二楚
- **能重复的绝不依赖个人** —— 任何人拿到代码都能重建整个环境

这些理念一旦内化了，看待技术问题的角度就完全不一样了。现在写任何东西，第一反应就是"这个能自动化吗？""这个有监控吗？""这个配置有版本管理吗？"
...
如果你正在考虑搞个 Homelab，别犹豫，找台旧电脑就能开始。三年后你会感谢今天的自己。
