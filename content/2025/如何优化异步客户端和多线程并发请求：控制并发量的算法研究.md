...
date = 2025-02-06
description = "异步客户端和多线程并发请求怎么控量。从信号量到 PID 控制器、令牌桶、自适应并发，每种算法用 Python 代码讲清楚原理和适用场景。"
...
answer = "asyncio.Semaphore 是协程级的，用在 async/await 里，不会阻塞线程；threading.Semaphore 是操作系统级的，会真把线程阻塞住。看你用的是异步还是多线程。"

[[extra.faq]]
question = "PID 控制器用在并发控制里靠谱吗？"
answer = "有效，但得耐心调参。P、I、D 三个系数没调好，并发量要么剧烈震荡，要么半天收敛不了。实际用的时候，先拿纯 P 跑起来，再慢慢加 I 和 D。"

[[extra.faq]]
question = "令牌桶和漏桶算法有什么区别？"
answer = "令牌桶允许突发——桶里攒了令牌就能一次用掉；漏桶严格按固定速率放行。要偶尔突发用令牌桶，要严格匀速用漏桶。"

[[extra.faq]]
question = "自适应并发调整策略适合什么场景？"
answer = "你不确定对面能扛多大并发、或者它的负载会变的时候，自适应最好用。它自己探上限，失败率一高就降速，比手动设参数灵活。"

[[extra.faq]]
question = "生产环境应该选哪种并发控制方案？"
answer = "大多数场景固定信号量就够，简单可靠。对面 API 有明确的 rate limit 就用令牌桶。负载不可预测、要自动适应，再上 PID 或自适应。别过度工程化。"
+++
